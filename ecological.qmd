  ...
  for (year in years) {
    ...
    if (year < 2006) {
      yearly_data <- yearly_data %>%
        select(-year, -perc_not_nt, -perc_exp6_pls, -perc_grad, -avg_exp_yrs) %>%
        rename(
          "p_bl" = "perblack",
          "p_hisp" = "perhisp",
          "p_wh" = "perwhite",
          "p_frl" = "perfrlunch",
          "d_seg_ne_wb" = "ns_wht_blk",
          "p_me_m3" = "m3_perc_me",
          "p_me_r3" = "r3_perc_me"
        )
      ...
      yearly_data <- yearly_data %>%
        ...
        rename(
          ...
          "p_frl" = "perfrlunch",
          "d_seg_ne_wb" = "ns_wht_blk",
          "p_tch_not_new" = "perc_not_nt",
          "p_tch_grad" = "perc_grad",
          "p_exp6_plus" = "perc_exp6_pls",
          "avg_exp" = "avg_exp_yrs"
        )
...
middl_corrs_fun <- function(data, years) {
  ...
  for (year in years) {
    ...
    if (year < 2003) {
      yearly_data <- yearly_data %>%
        select(-year, -perc_not_nt, -perc_exp6_pls, -perc_grad, -m7_perc_me, -r7_perc_me, -avg_exp_yrs) %>%
        rename(
          "p_bl" = "perblack",
          "p_hisp" = "perhisp",
          "p_wh" = "perwhite",
...
high_corrs_fun <- function(data, years) {
  ...
  for (year in years) {
    ...
    if (year < 2003) {
      yearly_data <- yearly_data %>%
        select(-year, -perc_not_nt, -perc_exp6_pls, -perc_grad, -m11_perc_me, -r10_perc_me, -perc_four_grad, -perc_four_drop, -avg_exp_yrs) %>%
        rename(
          "p_bl" = "perblack",
          "p_hisp" = "perhisp",
          "p_wh" = "perwhite",
      ...
      yearly_data <- yearly_data %>%
        select(-year, -perc_not_nt, -perc_exp6_pls, -perc_grad, -perc_four_grad, -perc_four_drop, -avg_exp_yrs) %>%  
        rename(
          "p_bl" = "perblack",
          "p_hisp" = "perhisp",
          "p_wh" = "perwhite",
      ...
      yearly_data <- yearly_data %>%
        ...
        rename(
          ...
          "p_exp6_plus" = "perc_exp6_pls",
          "avg_exp" = "avg_exp_yrs",
          "p_stu_grd" = "perc_four_grad",
          "p_stu_drpout" = "perc_four_drop"
        )
    } else {  # For years 2005 except 2019
      yearly_data <- yearly_data %>%
        select(-year) %>%
...
pelsb <- ecol %>% 
  ...
  select(
    ...
    teach_new = "NewTeacher", avg_exp_yrs = "AverageYearsofExperience", Experience1.5Years, Experience6.10Years, Experience11.15Years, Experience16.20Years, Experience21.25Years, Experience26.30Years, Experience31PlusYears, 
...
teach <- schools %>% 
  ...
  mutate(
    ...
    # Taking the complement of new to get those that are not new
    perc_not_nt = 1-perc_nt,
    # perc_exp15 = Experience1.5Years / teach_member,
    # The perc_exp6_pls is not the complement of perc_exp15
    # There is no consistent matching of sums across all experience rows that add up to the total FTE (even if we also add in the new teachers)
    perc_exp6_pls = (Experience6.10Years + Experience11.15Years + Experience16.20Years + Experience21.25Years + Experience26.30Years + Experience31PlusYears) / teach_member,
    # perc_nograd = (ExperienceLessThanBA + ExperienceBA) / teach_member,
    perc_grad = (ExperienceMA + ExperiencePhD) / teach_member,
...
elem <- corrs %>%
  ...
  select(
    ...
    # PELSB
    perc_not_nt, perc_exp6_pls, perc_grad, avg_exp_yrs,
    # MCA
...
#| echo: false
#| label: tbl-vars
#| tbl-cap: Variable list for correlation matrices
...
corrs %>% 
  ...
  mutate(
    description = case_when(
      variable=="p_bl" ~ "Percent Black",
      variable=="p_hisp" ~ "Pecent Hispanic/Latin@",
      variable=="p_wh" ~ "Percent white",
      variable=="p_frl" ~ "Percent free/reduced lunch",
      variable=="d_seg_ne_wb" ~ "White/Black Normalized Exposure Index",
      variable=="p_tch_not_new" ~ "Percent FTE's that are not new teachers",
      variable=="p_tch_grad" ~ "Percent of FTE's with a graduate degree (MA, PhD)",
      variable=="p_exp6_plus" ~ "Percent of FTE's with 6 or more years of experience",
      variable=="avg_exp" ~ "Average years of teacher experience",
      ...
      variable=="p_stu_grd" ~ "Percent of students that graduate in four years??",
      variable=="p_stu_drpout" ~ "Percent of students that drop out after four years??",
      TRUE ~ variable
    ),
    measure_level = case_when(
      variable=="p_bl" ~ "School",
      variable=="p_hisp" ~ "School",
      variable=="p_wh" ~ "School",
      variable=="p_frl" ~ "School",
      variable=="d_seg_ne_wb" ~ "District",
      variable=="p_tch_not_new" ~ "School",
      variable=="p_tch_grad" ~ "School",
      variable=="p_exp6_plus" ~ "School",
      variable=="avg_exp" ~ "School",
      variable=="p_me_m3" ~ "School",
      variable=="p_me_r3" ~ "School",
      variable=="p_me_m7" ~ "School",
      variable=="p_me_r7" ~ "School",
      variable=="p_me_m11" ~ "School",
      variable=="p_me_r10" ~ "School",
      variable=="p_stu_grd" ~ "School",
      variable=="p_stu_drpout" ~ "School",
      TRUE ~ variable
    ),
    source = case_when(
      variable=="p_bl" ~ "NCES",
      variable=="p_hisp" ~ "NCES",
      variable=="p_wh" ~ "NCES",
      variable=="p_frl" ~ "NCES",
      variable=="d_seg_ne_wb" ~ "Stanford Education Opportunity",
      variable=="p_tch_not_new" ~ "PELSB",
      variable=="p_tch_grad" ~ "PELSB",
      variable=="p_exp6_plus" ~ "PELSB",
      variable=="avg_exp" ~ "PELSB",
      ...
      variable=="p_stu_grd" ~ "MDE",
      variable=="p_stu_drpout" ~ "MDE",
      TRUE ~ variable
    )
  ) %>% 
  arrange(measure_level) %>% 
  gt() %>% 
  tab_header(
  ...
  cols_label(
    ...
    measure_level = "Measure Level",
    source = "Data Source",
  ) %>%
  gt_highlight_rows(
    rows = measure_level=="District",
    columns = everything(),
    fill = "#80bcd8",
    alpha = 0.2) %>% 
  tab_footnote(
    footnote = "Most variables are at the school level, except for the district-level segregation indices.",
    locations = cells_column_labels(
      columns = measure_level
    )
  ) %>% 
...
corrs %>% 
  ...
  select(
    ...
    perblack = "perblack.x", perhisp = "perhisp.x", perwhite = "perwhite.x", perfrlunch = "perfrlunch.x", ns_wht_blk,
    # PESLB
    perc_not_nt, perc_exp6_pls, perc_grad,
    # GRADUATION
    perc_four_grad, perc_four_drop,
    # MCA
    m3_perc_me, r3_perc_me, m7_perc_me, r7_perc_me, m11_perc_me, r10_perc_me) %>% 
...
#| fig-width: 12
#| fig-height: 8
#| fig-cap: Distribution of Average Teacher Experience for Twin Cities Metro Schools, 2018.
#| label: fig-avgexp
...
corrs %>% 
  ...
  ggplot() +
  aes(x = avg_exp_yrs, fill = level) +
  geom_histogram(alpha = 0.7) +
  scale_fill_manual(values = c("#264653", "#c1121f", "#e9c46a", "#f4a261", "#A092B7FF", "#8AD9B1FF", "#a7c957",  "#778da9", "#780000")) +
  labs(
    # Main title label
    title = "Distribution of Average Teacher Experience for Twin Cities Metro Schools, 2018",
    # Subtitle label
    subtitle = paste0("Stratified by <b style='color:#264653'>Elementary</b>, <b style='color:#e9c46a'>Middle</b>, and <b style='color:#c1121f'>High</b> Schools"),
    # X-axis label
...
#| fig-width: 12
#| fig-height: 8
#| fig-cap: Correlations for Twin Cities Elementary Schools, 2021.
#| label: fig-elemcorr21
...
#| fig-width: 12
#| fig-height: 8
#| fig-cap: Correlations Across Metro Midle Schools, 2021.
#| label: fig-middlcorr21
...
# high.03 <- high %>%
#   filter(year==2003) %>%
#   select(-year, -perc_grad, -perc_not_nt, -perc_exp6_pls, -avg_ , -perc_four_grad, -perc_four_drop) %>%
#   rename(
#     "p_bl" = "perblack",
#     "p_hisp" = "perhisp",
#     "p_wh" = "perwhite",
...
#| fig-width: 12
#| fig-height: 8
#| fig-cap: Correlations Across Metro High Schools, 2021.
#| label: fig-highcorr21
...
#| fig-width: 12
#| fig-height: 8
#| fig-cap: Correlations Between District-Level Normalized Epxosure Segregation and Ecological Variables, 2001-2021.
#| label: fig-timexample


elem_corrs %>% 
  filter(Var1=="d_seg_ne_wb") %>% 
  ggplot() +
  annotate(
    "rect",
    fill = "#6a994e",
  ...
  annotate(
    ...
    fill = NA, 
    label.color = NA,
    label = "<b>This is the correlation between the variable we are plotting and itself. As expected, it is consistely 1 across time.</b>",
    x = 2002, 
    y = 0.9,
    color = "#A092B7FF",
    size = 4,
  ...
  annotate(
    ...
    fill = NA, 
    label.color = NA,
    label = "<b>This is the correlation between the variable we are<br>plotting and p_me_m3. It is a negative correlation that is trending<br>towards 0 over time. This indicates that the negative<br>correlation between the variable we are plotting<br>(d_seg_ne_wb, in this example) and p_me_m3., is getting weaker<br>over time.</b>",
    x = 2001, 
    y = -0.3,
    color = "#2a9d8f",
    size = 3.5,
  ...
  labs(
    # Main title label
    title = "Correlations Between District-Level Normalized Epxosure Segregation and Ecological Variables, 2001-2021",
    # Subtitle label
...
elem_corrs %>% 
  filter(Var1 %in% c("d_seg_ne_wb", "p_bl", "p_hisp", "p_wh")) %>% 
  ggplot() +
  annotate(
    "rect",
    fill = "#6a994e",
...
See @tbl-vars for a list of variables.
...
high_corrs %>% 
  ...
  geom_line(linewidth = 1) +
  facet_wrap(~Var1) +
  scale_color_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#A092B7FF", "#8AD9B1FF", "#a7c957",  "#778da9", "#780000", "#c1121f", "#B93289FF", "#C86C7CFF", "#0D0887FF")) +
  geom_hline(
    yintercept = 0, 
    linewidth = 0.5,
    alpha = 0.5) +
