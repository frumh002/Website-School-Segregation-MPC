...
schools %>% 
  ...
  tab_style(locations = cells_column_spanners(spanners = "Percent of TC Metro"),
            style = cell_text(transform = "uppercase", v_align = "bottom", size = px(14), weight = 500)) %>%
...
distr.dems %>%
  filter(leaname %in% metro.districts2, year %in% c(1991, 2001, 2011, 2022)) %>%
  select(year, leaname, distr_bipoc) %>%
  group_by(year) %>% 
  mutate(perc = distr_bipoc / sum(distr_bipoc)) %>%
...
# Table of White Enrollment in Metro School Districts ------------------

tims.whits <- schools %>% 
  filter(leaname %in% metro.districts2) %>% 
  select(year, leaname, tot_white) %>% 
  mutate(tot_white = floor(tot_white)) %>% 
  group_by(year, leaname) %>%
  summarise(tot = sum(tot_white)) %>% 
  mutate(perc = round(tot/sum(tot), 3) *100) %>% 
  select(leaname, year, perc) %>%
  ungroup() %>% 
  complete(leaname, year, fill = list(perc = 0)) %>% 
...
schools %>% 
  ...
  mutate(perc = tot/sum(tot)) %>% 
  pivot_wider(names_from = "year", values_from = c("tot", "perc")) %>% 
  left_join(tims.whits, by = "leaname") %>% 
  arrange(desc(perc_2022)) %>% 
  mutate(perc_run = cumsum(perc_2022)) %>% 
  gt() %>% 
  fmt_percent(
  ...
  tab_header(
    title = "Twin Cities Metro School District White Student Enrollment Over Time",
    subtitle = "1991-2022 Enrollment from MDE Public File"
  ) %>% 
  opt_align_table_header(align = "center") %>% 
  cols_align(
