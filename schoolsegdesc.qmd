...
## Number of Schools in the TC Metro

:::{.callout-note}
## School Level Imputations
There is missing school level data for records before 2000. I imputed those school levels based on the level that was assigned to that school after 2000. It's uncommon, but some schools change levels over time. I imputed the school level based on the level that is most common over time for each missing school. This may create some innacuracies in school level analysis.
:::
...
schools %>% 
  ...
  group_by(year, level, leaname) %>% 
  summarise(
    tot = n()
  ) %>% 
  ggplot() +
  aes(x = year, y = tot, fill = level) +
  geom_col(
    position = position_dodge2(preserve = "single"),
    width = 1.5) +
  scale_fill_manual(values = c("#37659EFF", "#348FA7FF",  "#8AD9B1FF")) +
  scale_y_continuous(
    limits = c(0, 75),
  ...
  labs(
    ...
    # Subtitle label
    subtitle = paste0("<b style='color:#37659EFF'>Elementary/Primary Schools</b>, <b style='color:#8AD9B1FF'>Middle Schools</b>, and <b style='color:#348FA7FF'>High/Secondary Schools</b>"),
    # X-axis label
    x = "Fall of School Year",
    # Y-axis label
    y = "Number of Schools",
...
## Number of Schools Closed and Opened Over Time

::: {.callout-note}
## Limitations
A school closure is defined as any school that had students for at least one year followed by no students in a subsequent year. A school opening is defined as a school that had no students after 1991 but enrolled students before 2022 for at least one year. Some schools reopen after being closed, and only their initial closure is reflected in this analysis.
:::
...
# Which schools have at least one year of data and less than the full 32 years of data
# This implies that any school has not been open for at least one of the years between 1991 and 2022
closed <- schools %>% 
  left_join(school.levels, by = "schoolname", suffix = c("", ".df1")) %>%
  mutate(level = coalesce(level, level.df1)) %>%
  select(-level.df1) %>% 
  filter(leaname %in% metro.districts, level %in% c("Elementary/Primary", "Middle", "High/Secondary")) %>% 
  select(
    schoolname, leaname, year, level, member, 
    tot_nam, tot_asian, tot_hisp, tot_black, tot_tr, tot_white) %>% 
  complete(schoolname, leaname, year) %>%
  mutate(
    ch = case_when(is.na(member) ~ 0,
                   TRUE ~ 1)
  ) %>% 
  group_by(schoolname, leaname) %>% 
  summarise(
...
# Dataset created to select which schools were closed and 
sch.delt <- schools %>% 
  left_join(school.levels, by = "schoolname", suffix = c("", ".df1")) %>%
  mutate(level = coalesce(level, level.df1)) %>%
  select(-level.df1) %>% 
  ...
  select(
    ...
    tot_nam, tot_asian, tot_hisp, tot_black, tot_tr, tot_white) %>% 
  complete(schoolname, leaname, year) %>% 
  inner_join(closed, by = c("schoolname", "leaname")) %>%  # I only want to keep schools that were closed
  group_by(schoolname, leaname) %>% 
  arrange(schoolname, year) %>% 
  mutate(
    yr_open = min(year[!is.na(member)]),
    yr_close = max(year[!is.na(member)])
  ) %>% 
  ungroup()

# Plotting closures
sch.closed <- sch.delt %>% 
  filter(yr_close!=2022, year==yr_close) %>% 
  group_by(level, year) %>% 
  summarise(tot = n()) %>% 
  mutate(type = "closed")
# Seeing which school districts are most impacted
sch.closed.distr <- sch.delt %>% 
  filter(yr_close!=2022, year==yr_close) %>% 
  group_by(level, leaname, year) %>% 
  summarise(tot = n()) %>% 
  mutate(type = "closed")
# Plotting openings
sch.open <- sch.delt %>% 
  filter(yr_open!=1991, year==yr_open) %>% 
  group_by(level, year) %>% 
  summarise(tot = n()) %>% 
  mutate(type = "open")
# Merging together for plot
sch.change <- rbind(sch.closed, sch.open)
...
sch.change %>% 
  ...
  scale_x_continuous(breaks = seq(1991, 2022, by = 5)) +
  labs(
    # Main title label
    title = "Number of School Closures and Openings in the Twin Cities Metro, 1991-2022",
    # Subtitle label
...
level.yr.members <- schools %>% 
  left_join(school.levels, by = "schoolname", suffix = c("", ".df1")) %>%
  mutate(level = coalesce(level, level.df1)) %>%
  select(-level.df1) %>% 
  filter(leaname %in% metro.districts, level %in% c("Elementary/Primary", "Middle", "High/Secondary")) %>% 
  rename(tot_member = "member") %>% 
  mutate(across(contains("tot"), floor)) %>% 
  group_by(level, year) %>% 
  summarise(
...
# Plot of which school districts had the most closures over time
top.school.closures <- sch.closed.distr %>% 
  group_by(leaname) %>% 
...
top.school.closures.perc <- sch.closed.distr %>% 
  group_by(leaname) %>% 
  summarise(tot = sum(tot)) %>% 
  arrange(desc(tot)) %>% 
  mutate(
    sum = cumsum(tot),
    perc = round((sum/sum(tot))*100)) %>% 
  top_n(tot, n = 10) %>% 
...
::: {.callout-tip}
## Of Note
Intermediate School District 287 comprised 26% of all school closures between 1991-2022.
:::
...
sch.closed.distr %>% 
  mutate(leaname = case_when(leaname%in% top.school.closures ~ leaname,
                             TRUE ~ "Other")) %>% 
  ...
  scale_fill_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#e76f51", "#005300", "#780000", "#c1121f", "#fdf0d5",  "#ff99c8", "#a7c957", "#d0f4de", "#778da9", "#9b5de5")) +
  scale_x_continuous(breaks = seq(1991, 2022, by = 5)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    # Main title label
    title = "Proportion of School Closures by District in the Twin Cities Metro, 1991-2022",
    # Subtitle label
    subtitle = paste0("These school districts make up ", top.school.closures.perc, "% of Metro School Closures between 1992-2022<br>
                      <b style='color:#2a9d8f'>Intermediate School District 287</b>, <b style='color:#f4a261'>Minneapolis Public School District</b>, and <b style='color:#a7c957'>St. Paul Public School District</b> made up 51% of all closures"),
    # X-axis label
    x = "Fall of School Year",
    # Y-axis label
    y = "Number of Schools",
...
## Impact of School Closures on Students Across the Metro

::: {.callout-note}
## Limitations
Any estimates of counts of people affected by school closures will be imperfect because it's hard to account for schools that closed and reopened multiple times. With that said, this analysis selects only the first instance a school closed to estimate the number of students impacted.
:::
...
elementary.closed <- sch.closed %>% 
  group_by(level) %>% 
  summarise(tot = sum(tot)) %>% 
  filter(level=="Elementary/Primary") %>% 
  pull(tot)
middle.closed <- sch.closed %>% 
  group_by(level) %>% 
  summarise(tot = sum(tot)) %>% 
  filter(level=="Middle") %>% 
  pull(tot)
high.closed <- sch.closed %>% 
  group_by(level) %>% 
  summarise(tot = sum(tot)) %>% 
  filter(level=="High/Secondary") %>% 
  pull(tot)
...
```{r school_impact_plot}
#| echo: false
#| fig-width: 12
#| fig-height: 8

# Plot of impact of school closures on students
sch.delt %>% 
  filter(yr_close!=2022, year==yr_close) %>% 
  rename(tot_member = "member") %>% 
  mutate(
  ...
  summarise(
    ...
    tot_tr = sum(tot_tr),
    tot_white = sum(tot_white)) %>% 
  left_join(level.yr.members, by = c("level", "year")) %>% 
  mutate(
    perc_member = tot_member.x/tot_member.y,
    perc_nam = tot_nam.x/tot_nam.y,
    perc_asian = tot_asian.x/tot_asian.y,
    perc_hisp = tot_hisp.x/tot_hisp.y,
    perc_black = tot_black.x/tot_black.y,
    perc_tr = tot_tr.x/tot_tr.y,
    perc_white = tot_white.x/tot_white.y,
  ) %>% 
  select(level, year, contains("perc")) %>% 
  pivot_longer(c(-level, -year), names_to = "type", values_to = "perc") %>% 
  left_join(sch.closed %>% select(-type), by = c("level", "year")) %>% 
  arrange(level, year) %>% 
  mutate(
    type = case_when(type=="perc_asian" ~ "Asian",
                     type=="perc_hisp" ~ "Hispanic/Latin@",
                     type=="perc_black" ~ "Black/African American",
                     type=="perc_member" ~ "All Students",
                     type=="perc_white" ~ "White",
                     type=="perc_nam" ~ "Native American",
                     type=="perc_tr" ~ "Multiracial")
  ) %>% 
  ungroup() %>% 
  ggplot() +
  aes(x = year, y = perc, color = type, size = tot) +
  geom_point() +
  scale_color_manual(values = c("#0000FF", "#FF0000", "#00FF00", "#000033", "#FF00B6", "#009FFF", "#FFD300")) +
  scale_y_continuous(
    labels = scales::percent_format(accuracy = 1),
    limits = c(0, .08),
    breaks = seq(0, .08, by = 0.01)) +
  facet_wrap(~level) +
  labs(
    # Main title label
    title = "Students Affected by School Closures in the Twin Cities Metro, 1991-2022",
    # Subtitle label
    subtitle = paste0("Between 1991-2022, <b>", elementary.closed, " Elementary Schools, ", middle.closed, " Middle Schools, and ", high.closed, " High Schools</b> Closed As of 2022"),
    # X-axis label
    x = "Fall of School Year",
    # Y-axis label
    y = "Percent of Students (Race-Specific)",
    # Caption label
    caption = "Sources: Stanford NCES Data.",
    # Legend titles
    size = "Number of School Closures",
    color = "Race"
  ) +
  theme(
    # Legend location, formatting, and background
...
sch.delt %>% 
  ...
  left_join(sch.closed %>% select(-type) %>% rename(sch.cl = "tot"), by = c("level", "year")) %>% 
  arrange(level, year) %>% 
  mutate(
